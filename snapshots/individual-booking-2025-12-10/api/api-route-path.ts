...
export async function POST(req: Request) {
  try {
    ...
    try {
      data = JSON.parse(text) as MapboxDirectionsResponse;
    } catch {
    ...
    if (data.code !== "Ok") {
      const codeStr = data.code || 'Unknown';
      const errorMessage = mapboxCodeToMessage(codeStr);
      logError("Mapbox routing failed:", codeStr);
    ...
    logError("Route path error:", errorMessage);
